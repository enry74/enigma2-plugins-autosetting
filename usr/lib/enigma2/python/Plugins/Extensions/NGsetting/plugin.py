...
class MenuiSettingE2(Screen, ConfigListScreen):
	...
	#changed so that the upgrade is mandatory                   
        ...
        def hauptListEntryMenuA(self,name,type):				 
            res = [(name,type)]
            if HD.width() > 1280:
                res.append(MultiContentEntryText(pos=(190, 5), size=(300,40),font=0, text=name, flags=RT_HALIGN_CENTER))	 #fhd
...
def Plugins(**kwargs):
    return [PluginDescriptor(name = 'Vhannibal AutoSetting '+Version, description = 'Vhannibal Official Plugin by NGSetting', icon = 'Vhannibal.png', where = [PluginDescriptor.WHERE_EXTENSIONSMENU,PluginDescriptor.WHERE_PLUGINMENU], fnc = Main),
